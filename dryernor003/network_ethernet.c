...
static void ResetEthernet(void)
{
    ...
    if (Ethernet_Wifi_DualMAC == 0)
		...
		setting[0].dhcp = 1; //if eth+wifi daul work, use static IP.//emeng change 0 to 1 NET_WIFI_MAY_NOT_EXIST must open NET_WIFI_REDEFINE must close
	//setting[0].dhcp = 0; //if eth+wifi daul work, use static IP.
	printf("%s() dhcp = %d\n", __FUNCTION__, setting[0].dhcp);
