...
static void LoadScene(void)
{
	...
	//if (theConfig.lang != LANG_ENG)//emeng 
	if (theConfig.lang != 0)
...
void sceneTurnRightVoid(){

}
void sceneTurnLeftVoid(){

}
void sceneBtnPressVoid(){

}
void sceneBtnLeftandMidPress(){
	setbit(keyLongPress, 1);
	setbit(keyLongPress, 0);
}
void sceneBtnLeftPress(){
	setbit(keyLongPress, 0);
}
void sceneBtnMidPress(){
	setbit(keyLongPress, 1);
}
void sceneTurnRightUI(){
	printf("+++++\n");
	dryerKeyValue2 += 1;
}
void sceneTurnLeftUI(){
	printf("-----\n");
	dryerKeyValue2--;
}
void sceneBtnPressUI(){
	dryerKeyValue1 = 2;
}

static int winf2value = 0;
static int winf3value = 0;

static void winKeyAction(){
	//printf("%d,,,,%d\n", winf2value, winf3value);
	if (winf2value == 2){//按下
		setbit(keyLongPress, 0);
		
	}
	else if (winf2value == 0){//抬起
		clrbit(keyLongPress, 0);
	}
	...
	else if (winf3value == 0){//抬起
		clrbit(keyLongPress, 1);
	}
}
int SceneRun(void)
{
	...
	for (;;)
	{
		...
		while (SDL_PollEvent(&ev))
		{
			...
			case SDL_KEYDOWN:
				...
				{
							   //1
							   /*extern ITUBackground* BackgroundMenuArea11;
				...
				case SDLK_z:
				{
							   ...
							   memcpy(((u8*)&ui_tm) + 10, gbstr, sizeof(gbstr));
