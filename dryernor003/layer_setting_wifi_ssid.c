...
//emeng add //wifiMgr_is_wifi_available,ssid connect=true  gnApCount 0 = disconnected
//#ifdef CFG_NET_WIFI
//		nRet = wifiMgr_is_wifi_available(&bIsAvail);
//#endif
		if (gnApCount == 0){
			printf("%d,CANCEL PANEL DISPALY", gnApCount);
			/*CANCEL PANEL DISPALY*/
			memset(pList[j].ssidName, 0, 32);
			pList[j].rfQualityQuant = 0;
...
bool SettingWiFiSsidSignalScrollListBoxOnLoad(ITUWidget *widget, char *param)
{
    ...
    for (; j < getMaxSsidCount(); j++)
    {
		...
		//sprintf(buf, "%d%%", pList[j].rfQualityQuant);// 100%
		sprintf(buf, "%ddBm", pList[j].rfQualityRSSI);//emeng dbm
...
bool SettingWiFiSsidScrollListBoxOnSelect(ITUWidget *widget, char *param)
{
    ...
    if (scrolltext)
    {
        ...
        else
        {
            ...
            /*ituLayerGoto(settingWiFiPasswordLayer);*///emeng
        }
    }
    return true;
}
...
#else
        //gnApCount = wifiMgr_get_scan_ap_info(pList);//emeng 2s pause
#endif
    }
    else
    {
...
static int timer = 0;
bool SettingWiFiSsidOnTimer(ITUWidget* widget, char* param){

	if (timer++ >= 300){
		timer = 0;
		//getWifiListInfo();
		SettingWiFiSsidNameScrollListBoxOnLoad(settingWiFiSsidNameScrollListBox, "");
		SettingWiFiSsidStatusScrollListBoxOnLoad(settingWiFiSsidStatusScrollListBox, "");
		SettingWiFiSsidSignalScrollListBoxOnLoad(settingWiFiSsidSignalScrollListBox, "");
		return true;
	}
	return false;
}
//other layer //emeng end